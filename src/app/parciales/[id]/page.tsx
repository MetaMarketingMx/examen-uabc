...
function tituloDe(item?: Parcial | null) {
  return item?.nombre || item?.titulo || "Parcial";
}
...
export default function ParcialPage() {
  ...
  const parcialId = String(params.id);
  ...
  const [tiempoAsignadoMinutos, setTiempoAsignadoMinutos] = useState(30);
  ...
  async function cargarDatos() {
    ...
    const { data: parcialData } = await supabase
      .from("parciales")
      .select("*")
      .eq("id", parcialId)
      .single();
    ...
    const parcialInfo = (parcialData || null) as Parcial | null;
    const minutos = Number(parcialInfo?.tiempo_minutos || 30);

    setParcial(parcialInfo);
  ...
  async function terminarExamen(porTiempo: boolean) {
    ...
    const detalles: DetalleRespuesta[] = preguntas.map((pregunta) => {
      ...
      return {
        ...
        correcta: Boolean(respuestaUsuario) && respuestaUsuario.toUpperCase() === correcta,
    ...
    return (
      <main className="min-h-screen bg-slate-950 p-8 text-white">
        <div className="mx-auto max-w-4xl rounded-3xl border border-slate-800 bg-slate-900 p-8">
          <h1 className="text-3xl font-bold">{tituloDe(parcial)}</h1>
    ...
    return (
      <main className="min-h-screen bg-slate-950 p-8 text-white">
        <div className="mx-auto max-w-6xl rounded-3xl border border-slate-800 bg-slate-900 p-8">
          ...
          <h1 className="mt-2 text-4xl font-bold">{tituloDe(parcial)}</h1>

          <p className="mt-3 text-slate-400">
            Este es el resumen de tu examen. Puedes revisar tu calificación,
          ...
          {terminadoPorTiempo && (
            <div className="mt-5 rounded-2xl border border-red-500 bg-red-950/50 p-4 text-red-200">
              El examen terminó porque se agotó el tiempo asignado.
            </div>
          )}
            ...
            <div className="mt-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
              ...
              <p className="mt-2 text-slate-200">
                {estadisticas.porcentaje >= 90
                  ? "Excelente resultado. Dominas muy bien este parcial."
                  : estadisticas.porcentaje >= 70
                    ? "Buen resultado. Conviene revisar las preguntas donde tuviste error para reforzar el tema."
                    : estadisticas.porcentaje >= 60
                      ? "Resultado aprobatorio, pero se recomienda repasar antes de avanzar."
                      : "Se recomienda estudiar nuevamente este tema y volver a intentar el parcial."}
              </p>
            </div>
          </section>
          ...
          <section className="mt-8 rounded-3xl border border-slate-800 bg-slate-950 p-6">
            ...
            <div className="mt-5 grid gap-4">
              {preguntas.map((pregunta, index) => {
                ...
                const correcta = respuestaCorrectaDe(pregunta);
                const bien = Boolean(usuario) && usuario.toUpperCase() === correcta;
        ...
        <div className="mb-6 rounded-3xl border border-slate-800 bg-slate-900 p-6">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-bold">{tituloDe(parcial)}</h1>
